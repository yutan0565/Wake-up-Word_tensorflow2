...
def extract_features(in_files, in_y):
    ...
    for index, filename in enumerate(in_files):
        for target_word in Config.target_wake_word:
            ...
            # stretch stata 뺴주기
            if (target_word not in filename) or (Config.user_list[int(in_y[index])] not in filename) or ("stretch" in filename):
                continue
...
np.savez(Config.base_path +"user_set_multi_LSTM.npz",
         x_train=x_train,
         y_train=y_train,
         x_val=x_val,
         y_val=y_val,
